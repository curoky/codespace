...
该命令按 `resource.Dockerfile` 生成 `ghcr.io/curoky/codespace:workspace-resource`，与
base image 无关，只构建一次。payload 统一 stage 到镜像内的 `/opt/resource/...`（如
`/opt/resource/opt/rust`、`/opt/resource/usr/local/cuda-12.2`、`/opt/resource/opt/nvidia/*`、
`/opt/resource/usr/local/store/{radare2,rizin}`），镜像基于带 shell 和 coreutils 的
debian-slim，本身可作为执行环境。
